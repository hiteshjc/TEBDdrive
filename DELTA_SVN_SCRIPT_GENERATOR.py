...
###############################################################
# hz static field can be hacked to vary hz field
###############################################################
            ...
            for maxdim in maxdims:
                for N_tau in N_taus:
                    for numkick in numkicks:
                        ...
                        filename = "L_"+str(L)+"_In_TEBD_DELTA_with_SVN_experiment_site_1_kicked_hz_"+str(hz)+"_phi_"+str(
                        ...
                        f.write("cutoff=10^(-8)\n")
                        f.write("file_name=\"TEBD_DELTA_with_SVN_output_with_effective_ham_double_S_experiment_1_site_1_kicked_L_"+str(L)+"__hz_"+str(
                            hz)+"_phi_"+str(phi)+"_tau_"+str(tau)+"_maxdim_"+str(maxdim)+"_numkicks_"+str(numkick)+"_Ntau_"+str(N_tau)+"\"\n")
                        f.write("N_tau="+str(N_tau)+"\n")
                        f.close()

                        # should install julia locally on planck with version of ITensors
                        os.system("/home/rmelendrez/julia/julia entropy_hack_DELTA_TEBD_HPC_Staggered_Heisen.jl "+str(filename)+" > "+"pipe_out_DELTA_SVN_TEBD_output_experiment_1_site_1_kicked_L_"+str(
