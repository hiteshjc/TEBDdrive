...
################################################################################ 
	for j in 1:(N - 1)
		...
		local J = bonds_and_Js[j][3]
		local Jz = bonds_and_Js[j][4]
		...
		local hj = Jz* op("Sz", s1) * op("Sz", s2) + J* 1/2 * op("S+", s1) * op("S-", s2) +J * 1/2 * op("S-", s1) * op("S+", s2)- hz*op("Sz",s1)*op("Id",s2) + hx*op("Sx",s1)*op("Id",s2)
...
#     Maxbonddim at t₀ = 0
#     
#     SᵢˣSⱼˣ correlators calculation
#      Sᵢˣ local observable
#    appended to empty arrays
############################################################
                ...
                if n_tau > N_tau/2
                    ...
                    psi1 = normalize(psi1)
                        ...
                        if n_tau == N_tau
                            ...
                            # time appended at t = τ
                            push!(time,t)
                            
                            # Sᵢˣ Sⱼˣ correlation matrix	
                            local xxcorr= correlation_matrix(psi1,"Sx","Sx")
                            push!(XXmat,xxcorr)
                            
                            # Sᵢˣ local observable
                            local magz = expect(psi1,"Sx")
                            push!(Xmat,magz)
