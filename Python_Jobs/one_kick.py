...
#                        STOP
                        ...
                        j.write("export OMP_NUM_THREADS=48 \n")
                        #j.write("export MKL_NUM_THREADS=16 \n")
                        #j.write("#SBATCH -p genacc_q\n")
                        j.write("julia square_beta_tebd.jl "+str(filename)+" > "+"OPEN_pipe_out_Bhaskarian_pulse_site_"+str(kicks[0])+"_output_with_effective_ham_double_S_experiment_1_site_1_kicked_L_"+str(L)+"_hz_"+str(hz)+"_phi_"+str(phi)+"_tau_"+str(tau)+"_maxdim_"+str(maxdim)+"_numkicks_"+str(numkick)+"_Ntau_"+str(N_tau)+"\n")
                        j.close()
