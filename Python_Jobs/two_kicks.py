...
#                        STOP
                        ...
                        filename = "file_Bhaskarian_pulse_pulse_40_site_"+str(kicks[0])+"_site_"+str(kicks[1])+"_L_"+str(L)+"_40_pulses_In_Bhaskarian_pulse_site_1_hz_"+str(hz)+"_phi_"+str(phi)+"_tau_"+str(tau)+"_maxdim_"+str(maxdim)+"_numkicks_"+str(numkick)+"_Ntau_"+str(N_tau)
                        f = open(filename, 'w')
                        f.write("L="+str(L)+"\n")
                        ...
                        f.write("cutoff=10^(-8)\n")
                        f.write("file_name=\"OPEN_Bhaskarian_pulse_pulse_40_site_"+str(kicks[0])+"_site_"+str(kicks[1])+"_output_with_effective_ham_double_S_experiment_1_site_1_kicked_L_"+str(L)+"__hz_"+str(hz)+"_phi_"+str(phi)+"_tau_"+str(tau)+"_maxdim_"+str(maxdim)+"_numkicks_"+str(numkick)+"_Ntau_"+str(N_tau)+"\"\n")
                        ...
                        j.write("#SBATCH --cpus-per-task=8\n")
                        j.write("#SBATCH --mem=128G\n")
                        j.write("export OMP_NUM_THREADS=16 \n")
                        #j.write("export MKL_NUM_THREADS=16 \n")
                       # j.write("#SBATCH -p genacc_q\n")
                        j.write("julia square_beta_tebd.jl "+str(filename)+" > "+"OPEN_pipe_out_OPEN_Bhaskarian_pulse_pulse_40_site_"+str(kicks[0])+"_site_"+str(kicks[1])+"kicked_L_"+str(L)+"__hz_"+str(hz)+"_phi_"+str(phi)+"_tau_"+str(tau)+"_maxdim_"+str(maxdim)+"_numkicks_"+str(numkick)+"_Ntau_"+str(N_tau))
                        j.close()
