...
println(to)
	t≈ttotal && break
	global psi1 = apply(gates, psi1; cutoff,maxdim)
	normalize(psi1)
            #println("prakash t mod tau = ",t%tau, "   prakash t= ",t)

		if n%N_tau == 0.0 && n > 0
		#	println("t = ", t,"     t/τ= ",t/tau)
			for foot in kick
			psi1= apply(foot,psi1;cutoff=cutoff,maxdim=maxdim)
			normalize(psi1)
			end  
